...
interface LocationStatCardsProps {
  total: number;
  active: number;
  inactive: number;
  isLoading: boolean;
...
const CARDS = (total: number, active: number, inactive: number) => [
  { title: 'Total Locations',    value: total,    icon: <LocationOn />,  color: '#3B82F6' },
  { title: 'Active Locations',   value: active,   icon: <CheckCircle />, color: '#10B981' },
  { title: 'Inactive Locations', value: inactive, icon: <Cancel />,      color: '#EF4444' },
];

const LocationStatCards: React.FC<LocationStatCardsProps> = ({ total, active, inactive, isLoading }) => (
  <Box sx={{
    display: 'grid',
...
export default LocationStatCards;
